...
async function checkId(req, res, next) {
  ...
  if (!data)
    return next({ status: 404, message: `Error: Table ${table_id} not found` });
  else {
...
async function validateNewTable(req, res, next) {
  if (!req.body.data) return next({ status: 400, message: 'Error: Data missing' });

  const { table_name, capacity, reservation_id } = req.body.data;

  if (!table_name || table_name === '' || table_name.length === 1)
    return next({ status: 400, message: 'Error: var table_name must be valid' });

  if (!capacity || capacity === 0 || typeof capacity !== 'number')
    return next({ status: 400, message: 'Error: var capacity must be valid' });
...
async function validateUpdate(req, res, next) {
  ...
  if (!reservation_id)
    return next({ status: 400, message: 'Error: var reservation_id must exist' });

  const reservation = await reservationService.read(reservation_id);
  if (!reservation)
    return next({ status: 404, message: `Error: Reservation ${reservation_id} does not exist` });

  if (reservation.status === 'seated')
    return next({ status: 400, message: 'Error: var status is already seated' });
...
async function validateCapacity(req, res, next) {
  ...
  if (table.capacity < reservation.people)
    return next({ status: 400, message: `Error: Table ${table.table_name} does not have the capacity to seat ${reservation.people} people.` });

  if (table.occupied)
    return next({ status: 400, message: `Error: Table ${table.table_name} is currently occupied.` });
...
async function destroy(req, res, next) {
  ...
  if (!table.occupied)
    return next({ status: 400, message: `Error: Table ${table.table_name} is not occupied.` });
